...
| NL routing | `router.ts` — the LLM skill-router (a core product feature): it reads a free-form chat message and routes it to ONE of the 17 skills / intake / clarify, and also re-ranks the skills popover (advisory only). Single `emit_result` + Zod, fail-closed → clarify. |
