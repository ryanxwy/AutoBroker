...
1. **Layer boundaries** (`ARCH_OVERVIEW.html`): the five layers import one-way only
...
2. **Side-effect path** (`ARCH_GATE_STACK.html` + `INVARIANTS.html`): every external
...
5. **Decisions** (`DECISIONS.html`): flag code that contradicts a locked decision
   (e.g. defaulting to a non-DeepSeek provider when DeepSeek is the documented
   default; reintroducing MCP transport; using `ai@^7`).
6. **driver_kind namespace** (`ANCHORS.html`): the product `DriverKind` enum is
...
`PHASE_*.html` / `DECISIONS.html` entry should be updated (do not edit it yourself —
