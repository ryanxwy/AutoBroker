...
its HEADED windows are EXPECTED.** It scans the two launch shopping sites (Cars.com +
Edmunds) through their public filter UIs — ~1 SRP load per site, no detail pages, no
pagination, ≤10 kept listings — and deliberately opens VISIBLE serial browser windows
(headless UAs are edge-blocked; the honest posture is a real window, zero UA/fingerprint
masquerade). Windows appearing mid-run are correct behavior — neither the driver nor the
ui-monitor flags them.
...
`intake → dealer_geosearch → inventory_site_scan → inventory_aggregator_scan →
...
4. **inventory_aggregator_scan** · `/inventory_aggregator_scan` · inventory tab · (no gate — read-only shopping-site scan; the 18th skill, 2026-07-03) — run RIGHT AFTER site_scan (order B) so the cross-source seam is live: an already-stored VIN is skipped, existing site_scan rows are enrich-only (their `source_id`/price NEVER flipped; no cross-source price_change), new rows land with `source_type='aggregator_srp'` and render the muted `inventory-source-line` ("via <host>") on the inventory tab. **HEADED serial browser windows are EXPECTED** (edge-blocked headless UA; honest posture — never flag the visible windows; a per-site `blocked` outcome is an environment signal, not a crash). NL phrasing (PASS-A): "search shopping sites like Cars.com and Edmunds for my car" — plain "scan dealer sites" still routes to site_scan (correct, not a bug). **VERDICT = the voiced run summary, NOT dataquality** (no dataquality branch — `references/harness-boundaries.md`): "Kept N exact-match" + the voiced drop parens ("M didn't match your exact search", "K not yet in stock") + per-site failures voiced ("blocked automated scanning" / "couldn't confirm your location — skipped its results this run"; `blocked`/`location_not_applied` are the INTERNAL outcome codes, never on-screen text — grep the chat for the quoted strings). **Suspect-0 heuristic:** Kept==0 while site_scan holds ≥3 in-radius exact matches on the same profile → BACKLOG (match-pipeline regression candidate: the extract-prompt model-field grounding / `resegmentModelTrim` belt); Kept==0 with site_scan also thin/0 → a legit thin market. Launch adapters are Cars.com + Edmunds ONLY (iSeeCars absent by design). Aggregator rows are EXCLUDED from `dataquality?skill=inventory_site_scan` so site_scan's price/breakdown coverage stays clean.
...
never a silent fabrication; see item 10) — the single highest-value check; (2) **a doc-fee flag fires** — `DOC_FEE_CAP` in a capped metro (CA/NY/WA +
...
- **18th skill `inventory_aggregator_scan` (2026-07-03 wave)** — see item 4: headed windows EXPECTED (never a finding); the voiced kept/dropped summary is the verdict surface; apply the suspect-0 heuristic; and confirm `dataquality?skill=inventory_site_scan` still reads site_scan-only metrics (aggregator rows excluded — a site_scan 0-yield plus aggregator rows must NOT fire the F1 breakdown rule).
