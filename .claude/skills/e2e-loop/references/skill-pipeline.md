...
ui-monitor flags them. **It normally arrives as the AUTO-CHAINED sibling** a completed
`inventory_site_scan` fires for the same profile (the parent turn voices `Also checking
shopping sites (Cars.com, Edmunds)…` and a second assistant turn streams the aggregator run)
— you rarely launch it by hand; see §B and item 4.
