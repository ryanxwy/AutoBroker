...
16. **dealer_closeout_email** · `/dealer_closeout_email` · receipt + suppress · `approval-approve`/`batch-skip-all` — run SECOND-LAST (closes the profile); count fake sends ("0 sent" was a bug; under AUTOBROKER_MODE=test (fake-send) writes 0 fake_mailbox); decline=Δ0; atomic send+close+suppress; SKIP-ALL typed-return.
