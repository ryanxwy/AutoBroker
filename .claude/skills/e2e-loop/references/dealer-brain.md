...
- **Realism over cost.** Spend the LLM calls; do the full negotiation + full
  pre-flight market search (geosearch + site_scan + aggregator_scan + compare +
  incentives).
...
| `reply_extract` | `dealer_quotes` grows; revised OTDs grind to the floors. **Lane A (`--provider deepseek`): structured-output CLEAN — or, if `emit_result` never fires, fail-closed with a thrown `EmitResultNotCalledError` / `ZodError` + one ledgered failReason row** (no retry lane, no retry button — see `skill-pipeline.md` item 10). **Lane B (`--provider claude`): typed rows, or a ledgered fail-closed row — structurally exempt from the structured+tools mixing bug, no recovery hop.** On either lane a silent tool-SKIP / regex-exec'd name IS still the blocker; check on the largest extraction |
