...
and it must **NOT fabricate a pick**. Exposes a boundary the P1–P8 rotation never
...
|---|---|---|---|
| overview | `canvas-tab-overview` | `canvas-next-actions`, `canvas-overview-empty`, `canvas-summary-best-otd`, `canvas-summary-headline` | Plain-language copy; one-number-one-home vs digest line |
...
- Workbench layout is ONE always-on split (there is NO canvas/chat MODE toggle —
  ...
  **Viewport == window:** drive the browser at a viewport NO WIDER than the visible window
  ...
  `clientWidth` exceed the window, so a persisted rail-width clamps against the OVER-WIDE
  document and the chat visually clips — a HARNESS artifact, NOT a product bug. `App.tsx`
  already re-clamps a persisted width against the live container on mount + resize, so a
  real buyer (viewport == window) never clips. (PIC-20260627r3-3 was this artifact —
  `live-verified, no product change`.)
...
## Seasoned cases (advisory) — paired with shipped fixes

Each is an **advisory** edge probe layered ON TOP of a fix's deterministic regression
(it NEVER blocks CI). Run it when its CONDITIONAL trigger is independently drawn — never
