...
/**
 * GateCardSwitch.test — the surface-agnostic gate dispatch + the never-hidden
 * safety floor. batch_review renders its REAL decision surface (BatchReviewCard)
 * and wires submit/decline onto the decide() controller; the inbox + hygiene
 * shapes route by SHAPE to their own cards (never misrouting each other); an
 * approval suspend carrying a `summary` renders the REAL ApprovalPrompt; a
 * confirmation_gate suspend renders the REAL destructive typed-YES card. A
 * summary-less approval and any MALFORMED payload fall back to the visible
 * pending placeholder with zero decision controls (a pending gate is never
 * silently hidden, never mis-rendered).
 *
 * This component owns the dispatch on BOTH surfaces (banner + rail), so its card
 * behavior is exercised here once, independent of mount surface.
 */
...
/** The projected pending suspend for a gate of `kind`. */
...
describe("GateCardSwitch — the batch_review decision surface", () => {
  it("a batch_review suspend renders the REAL card (no placeholder) and submit posts accept{approved_dealer_ids}", () => {
    ...
    const r = render(<GateCardSwitch awaiting={awaiting("batch_review", BATCH_SPEC)} decision={decision} />);
  ...
  it("a MALFORMED batch_review payload falls back to the never-hidden pending placeholder", () => {
    const r = render(
      <GateCardSwitch awaiting={awaiting("batch_review", { targets: "not-an-array" })} decision={controller()} />,
    );
...
describe("GateCardSwitch — the inbox decision surface + three-reader disambiguation", () => {
  it("an inbox batch_review renders the InboxReviewCard (not the inventory card, not the placeholder)", () => {
    const decision = controller();
    const r = render(<GateCardSwitch awaiting={awaiting("batch_review", INBOX_SPEC)} decision={decision} />);
    expect(r.query("gate-banner-pending")).toBeNull();
    expect(r.query("batch-review-card")).toBeNull();
  ...
  it("an inventory batch_review still routes to the BatchReviewCard (no inbox misroute)", () => {
    const r = render(<GateCardSwitch awaiting={awaiting("batch_review", BATCH_SPEC)} decision={controller()} />);
  ...
  it("a hygiene batch_review routes to the HygieneReviewCard (no inbox misroute)", () => {
    const r = render(<GateCardSwitch awaiting={awaiting("batch_review", HYGIENE_SPEC)} decision={controller()} />);
    expect(r.query("inbox-review-card")).toBeNull();
...
describe("GateCardSwitch — the destructive confirmation_gate surface", () => {
  it("a confirmation_gate suspend renders the REAL typed-YES card (no placeholder); Confirm posts accept{confirm_token:'YES'}", () => {
    const decision = controller();
    const r = render(<GateCardSwitch awaiting={awaiting("confirmation_gate", CONFIRMATION_SPEC)} decision={decision} />);
    expect(r.query("gate-banner-pending")).toBeNull();
...
describe("GateCardSwitch — the approval decision surface + pending floor", () => {
  it("a SUMMARY-LESS 'approval' suspend renders the visible pending card with no decision controls", () => {
    const r = render(<GateCardSwitch awaiting={awaiting("approval")} decision={controller()} />);
  ...
  it("a summary-carrying approval renders the REAL ApprovalPrompt (no placeholder); Approve posts accept", () => {
    const decision = controller();
    const r = render(<GateCardSwitch awaiting={awaiting("approval", APPROVAL_SPEC)} decision={decision} />);
    expect(r.query("gate-banner-pending")).toBeNull();
