...
describe("groupSkillsByReadiness — profile-ASK UI projection", () => {
  it("with no pin: intake alone is ready, everything else is blocked", () => {
    const groups = groupSkillsByReadiness([intake, other], null);
    expect(groups.ready.map((s) => s.name)).toEqual(["search_profile_intake"]);
    expect(groups.blocked.map((s) => s.name)).toEqual(["quote_audit"]);
  });

  it("with an active pin: every skill is ready", () => {
    const groups = groupSkillsByReadiness([intake, other], "p1");
...
describe("SkillsPopoverList — rendered Run buttons", () => {
  it("no pin: intake Run is a real enabled <button>, the other is disabled; intake fires onRun", () => {
    const onRun = vi.fn();
    const r = render(<SkillsPopoverList skills={[intake, other]} activePin={null} onRun={onRun} />);
  ...
  it("with a pin: the non-intake Run enables and fires onRun", () => {
    const onRun = vi.fn();
    const r = render(<SkillsPopoverList skills={[intake, other]} activePin="p1" onRun={onRun} />);
    const otherRun = r.get("ledger-run-quote_audit") as HTMLButtonElement;
